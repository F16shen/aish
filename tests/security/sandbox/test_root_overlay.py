...
from aish.security.sandbox import SandboxConfig, SandboxExecutor, SandboxUnavailableError
...
def test_root_overlay_tmp_mount_failure_does_not_fallback(monkeypatch):
    ...
    monkeypatch.setattr(executor, "_list_system_root_overlay_targets", lambda: [Path("/tmp")])

    def fail_mount(*, lowerdir, upperdir, workdir, merged):
        raise SandboxUnavailableError("overlay_mount_failed", details="operation not permitted")

    monkeypatch.setattr(executor, "_mount_overlay", fail_mount)

    def should_not_run(*args, **kwargs):
        raise AssertionError("fallback should not continue to bubblewrap after overlay failure")
...
def test_root_overlay_syncs_upper_metadata_from_lower(monkeypatch):
    ...
    def capture_mount(*, lowerdir, upperdir, workdir, merged):
        ...
        raise SandboxUnavailableError("overlay_mount_failed", details="stop-after-check")
